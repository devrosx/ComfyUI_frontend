...
export const useExecutionStore = defineStore('execution', () => {
  ...
  function keepPartialStates(
    ...
    if (workflow) partialStatesByWorkflow.set(toRaw(workflow), states)
    if (!workflow || toRaw(workflow) === toRaw(workflowStore.activeWorkflow)) {
      nodeProgressStates.value = states
    }
  }
  ...
  function handleExecutionSuccess(e: CustomEvent<ExecutionSuccessWsMessage>) {
    const jobId = e.detail.prompt_id
    pendingExecutionErrorsByJobId.delete(jobId)
    const workflow = jobIdToWorkflow.get(jobId) ?? workflowStore.activeWorkflow
